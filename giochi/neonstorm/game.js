...
/* ══════════════════════════════════════════════════════════════
   NEONSTORM // Protocollo Zero — LOGICA DI GIOCO (v1.3)
   ══════════════════════════════════════════════════════════════ */
'use strict';
/* ════════ UTILS ════════ */
const TAU=Math.PI*2;
...
function resize(){
  ...
  player.x=clamp(player.x,14,W-14);player.y=clamp(player.y,64,H-16);
}
...
function updatePlayer(dt){
  ...
  const spd=player.focus?155:365;
  player.x=clamp(player.x+dx*spd*dt,14,W-14);
  player.y=clamp(player.y+dy*spd*dt,64,H-16);
  player.inv-=dt;
...
/* ★ FIX v1.3 — l'array eBullets viene modificato SOLO da questo loop.
   Prima playerHit() eseguiva swap-pop dentro l'iterazione: i proiettili
   venivano spostati/cancellati per errore (sparizioni) o aggiornati due
   volte nello stesso frame (teleport + colpi fantasma). Ora i proiettili
   vengono solo marcati con b.dead e rimossi qui, in ordine e senza conflitti. */
function updateEBullets(dt){
  ...
  for(let i=eBullets.length-1;i>=0;i--){
    ...
    if(player.alive&&player.inv<=0&&G.state==='play'){
      ...
      if(d2<rr*rr){rm(eBullets,i);playerHit();continue;}   // prima rimuovi, poi gestisci il danno
      const gr=rr+15;
...
function playerHit(){
  ...
  /* ★ FIX: marca soltanto, nessuna rimozione diretta dell'array */
...
function rotDraw(s,x,y,ang,w,h){
  ...
  baseT();   // ★ FIX v1.4 — ripristina la trasformazione base dopo ogni disegno ruotato
}
